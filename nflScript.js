...
function isFootballActive() {
    let today = new Date();
    let year = today.getFullYear();

    // Get the inactive period
    let { startDate, endDate } = getNFLInactivePeriod(year);

    // Football is active if the current date is **before** startDate or **after** endDate
    return today < startDate || today > endDate;
}

// Helper function to get the inactive period
function getNFLInactivePeriod(year) {
    // Get Super Bowl date (2nd Sunday of February)
    let superBowl = new Date(year, 1, 1); // February 1st
    while (superBowl.getDay() !== 0) { // Find first Sunday
        superBowl.setDate(superBowl.getDate() + 1);
    }
    superBowl.setDate(superBowl.getDate() + 7); // Move to 2nd Sunday

    // Start date: 2 weeks after Super Bowl
    let startDate = new Date(superBowl);
    startDate.setDate(startDate.getDate() + 14);

    // Find first Thursday of August
    let preSeasonStart = new Date(year, 7, 1); // August 1st
    while (preSeasonStart.getDay() !== 4) { // Find first Thursday
        preSeasonStart.setDate(preSeasonStart.getDate() + 1);
    }

    // End date: 2 weeks before first Thursday of August
    let endDate = new Date(preSeasonStart);
...
// Example usage:
console.log(isFootballActive()); // Returns true if football is in season, false otherwise
